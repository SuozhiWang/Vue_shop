 ...
 * @LastEditors: wsz
 * @LastEditTime: 2021-06-04 10:38:49
-->
<template>
  <div>
    <!-- 面包屑导航区 -->
...
export default {
  ...
  methods: {
    ...
    async removeRightsById(role, rightId) {
      // 弹窗提示用户是否进行删除
      const confirmResult = await this.$confirm('此操作将永久删除该权限, 是否继续?', '警告', {
      ...
      // 如果调用了getRoleList()会直接重新渲染页面，不建议使用
      //this.getRoleList()
    ...
    async showSetRightDialog(role) {
      ...
      // 把获取到的权限数据赋值给rightsList数组
      this.rightsList = res.data
      // console.log(this.rightsList)
      this.getLeafKeys(role, this.defKeys)
      this.setRightDialogVisible = true
    },
    // 通过递归的方式，获取角色下的所有三级权限的id并保存到数组defKeys中
    getLeafKeys(node, arr) {
      if (!node.children) {
        // 如果当前节点不包含children数据节点，则其为三级节点
