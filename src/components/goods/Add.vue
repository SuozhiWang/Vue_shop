...
<template>
  <div>
    ...
    <el-dialog title="图片预览" :visible.sync="previewVisible" width="50%">
      <img :src="preiewPath" alt="" class="previewImg" />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 图片预览触发的函数
    handlePreview(file) {
      this.preiewPath = file.response.data.url
    ...
    handleRemove(file) {
      ...
      // 3. 调用数组的splice()方法，把图片信息对象，从pics中移除
    ...
    add(){
      this.$refs.addFormRef.validate(async valid => {
        ...
        // 执行添加的业务逻辑
        // lodash  cloneDeep(obj)  用于深拷贝一个对象
