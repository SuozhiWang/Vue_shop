...
export default {
  ...
  methods: {
    ...
    handleInputConfirm(row) {
      ...
      // 如果没有return出去 则说明用户输入了正确的内容，需要做后续处理
      row.attr_vals.push(row.inputValue.trim())
      row.inputValue = ''
      row.inputVisible = false
      this.saveAttrVals(row)
    },
    // 将attr_vals的数据持久化到数据库中
    async saveAttrVals(row){
      // 需要发起请求，将添加的数据持久化到数据库中
      const { data: res } = await this.$http.put(
    ...
    showInput(row) {
      ...
      // 让文本框自动获得焦点
      // $nextTick方法的作用就是当页面上的元素被重新渲染之后，才会指定回调函数中的代码
      this.$nextTick((_) => {
